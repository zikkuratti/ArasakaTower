...
/*Комментарий для n0ble: В оригинале функции estimateDualDexTrade и  SpiceHarvester такого вида как ниже в этом комменте
 ...
 Компилятор солидити начал ругаться что стек слишком глубокий, обычно предлогают решать через структуры и массивы что 
 ...
 в данном контракте структура вынесена за функции и пытается из мэмори пихать их в функции но так как harvester.js вызывает переменные 
 в функциях то не достукивается до структуры находящейся внутри контракта за функцией  - тогда как это задать корректно непонятно? 
 Сразу же структурой из js кидать?
